...
import { Edit, CheckCircle2, XCircle, Plus } from "lucide-react";
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Gestionați șabloanele pentru generarea automată de sarcini pentru
          companiile dvs.
          {!isAdmin && (
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  {template.is_active ? (
                    <Badge
                      variant="outline"
                      className="bg-green-50 text-success border-success gap-1"
                    >
                    ...
                    <Badge
                      variant="outline"
                      className="bg-gray-50 text-muted-foreground gap-1"
                    >
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  ...
                  <Badge variant="outline">
                    ...
                    {template.deadline_month &&
                      ` din Luna ${template.deadline_month}`}
                  </Badge>
                  ...
                  {template.applies_to_org_types &&
                    template.applies_to_org_types.length > 0 && (
                      <Badge variant="outline">
                        {template.applies_to_org_types.join(", ")}
                      </Badge>
                    )}
                </div>
                {template.reminder_days &&
                  template.reminder_days.length > 0 && (
                    <p className="text-xs text-muted-foreground">
                      Reamintiri: {template.reminder_days.join(", ")} zile
                      înainte
                    </p>
                  )}
              </div>
